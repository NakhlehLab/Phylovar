...
#=========================================================================================== #
def ML(tree, names, mu0, mu1, MD, one_Ls_, zero_Ls_):
    s_time = time.time()
...
#================================================================ #
def best_split(tree, names, mu0, mu1, MD, one_Ls_, zero_Ls_):
...
def Search_sequential(arguments):
    [args, names, fns, fps, thread_num] = arguments
    ...
    if args.vio>0:
        ...
        (new_mat, Likelihood) = ML(tree=ete_nj_init, names=names, mu0=1e-3, mu1=0.5, MD=args.mdthr, one_Ls_=current_one_mat,zero_Ls_=current_zero_mat)
    ...
    while (waiting_counter<waiting_iter and stochastic==0) or (iter_counter<n_iterations and stochastic==1):
        ...
        elif iter_counter%3==1:
            # =================================== #
            # select a random false-positive rate #
            # =================================== #
            fp_idx = random.choice(range(len(fps)))
            t_alpha = fps[fp_idx]
            t_zero = zeros[fp_idx]

        else:
            # =================================== #
            # select a random false-netagive rate #
            # =================================== #
            fn_idx = random.choice(range(len(fns)))
            t_beta = fns[fn_idx]
            t_one = ones[fn_idx]
...
def Search(arguments):
    ...
    if args.vio>0:
        ...
        (new_mat, Likelihood) = ML(tree=ete_nj_init, names=names, mu0=1e-3, mu1=0.5, MD=args.mdthr, one_Ls_=one_mat,zero_Ls_=zero_mat)
    print("computing the best likelihood took %s seconds" %(time.time()-st))
    # the best likelihood value so far
    best_L = Likelihood
    # the current likelihood for tree search
...
if __name__ == "__main__":
    ...
    if args.fp == None:
        ...
        # fps_ = np.array([0.0000001,0.000001,0.00001,0.0001,0.001,0.01,0.05, 0.1])
    ...
    if not args.seq:
        ...
        for fp in fps_:
            for fn in fns_:
                for t_ in range(args.c):
                    ...
                    # ==================================================== #
                    pool_args.append([args, names, fn, fp, t_+1])
        pool = mp.Pool(args.M)
        ...
        for t_ in range(args.c):
            ...
            # ======================================== #
            pool_args.append([args, names, fns_, fps_, t_+1])
        pool = mp.Pool(args.M)
    ...
    if not args.seq:
        ...
        for t_ in range(args.c):
            if L_dict[t_][-1]>max_:
                ...
                max_ = L_dict[t_][-1]
        print("the best likelihood is %s for false-negative rate %s and false-positive rate %s" %(max_, best_index[0], best_index[1]))
        print("table of reports from threads\n-----------------------------------------------------------")
        for t_ in range(args.c):
            print("best likelihood from thread %s: %s, %s, %s" %(t_+1,L_dict[t_][-1],a_dict[t_][-1],b_dict[t_][-1]))
        print("-----------------------------------------------------------")
    ...
    init_noparams = init_noparams.astype(np.float128)
    ...
    fp_est = float(len(fp_idx)) / float(len(non_missing_idx))
